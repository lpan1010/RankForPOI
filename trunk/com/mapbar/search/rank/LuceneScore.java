...
public class LuceneScore {
	...
	public void init(List<POIObject> list){
		...
		maxScore = list.get(0).getScore();
		/**
		 * 求最大值和最小值
		 */
		for(POIObject poiObject : list){
			float poiScore = poiObject.getScore();
	...
	public float Normalization(POIObject poiObject){
		 ...
		 */
		float newScore = 0.0f;
		double poiScore = poiObject.getScore();
		/**当最大值等于最小值，说明luceneScore都是一样的，随便赋予一个值，因为此时luceneScore已经对排序没有意义了*/
		if(maxScore == minScore ){
			newScore = 0.5f;
		}
		else{
			newScore = (float) ((float)(poiScore-minScore)/(maxScore-minScore));
